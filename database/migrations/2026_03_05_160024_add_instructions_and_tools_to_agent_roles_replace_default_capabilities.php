...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        foreach (DB::table('agent_roles')->whereNotNull('default_capabilities')->get() as $row) {
            $capabilities = json_decode($row->default_capabilities, true);
            if (is_array($capabilities)) {
                DB::table('agent_roles')
                    ->where('id', $row->id)
    ...
    public function down(): void
    {
        ...
        foreach (DB::table('agent_roles')->whereNotNull('tools')->get() as $row) {
